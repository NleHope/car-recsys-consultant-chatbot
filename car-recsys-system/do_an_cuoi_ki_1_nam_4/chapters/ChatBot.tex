...
\section{Mô hình cơ sở xây dựng chatbot -- Agentic RAG}

Trong dự án này, nhóm sử dụng mô hình \textbf{RAG} (Retrieval-Augmented Generation) làm nền tảng, nhưng nâng cấp lên kiến trúc \textbf{Agentic RAG} sử dụng \textbf{LangGraph} để xử lý đa dạng loại câu hỏi. RAG kết hợp giữa \textbf{truy xuất thông tin} và \textbf{sinh ngôn ngữ}, tạo câu trả lời dựa trên dữ liệu thực tế thay vì chỉ dựa vào kiến thức sẵn có của mô hình.
...
Điểm khác biệt so với RAG thuần túy là hệ thống được xây dựng trên \textbf{LangGraph StateGraph} với 12 nodes và \textbf{conditional routing} theo ý định câu hỏi. Mỗi request đi qua một đồ thị trạng thái \texttt{AgenticState} chứa: lịch sử hội thoại, câu hỏi hiện tại, context đã truy xuất, ý định phân loại, thông tin hồ sơ người dùng và danh sách xe kết quả.
