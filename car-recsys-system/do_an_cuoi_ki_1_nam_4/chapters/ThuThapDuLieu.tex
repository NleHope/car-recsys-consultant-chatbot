\section{Xác định thông tin cần lưu trữ}
...
\section{Tiến hành thu thập dữ liệu}

\subsection{Vượt qua cơ chế chống bot (Cloudflare Turnstile)}
...
\subsubsection{Giai đoạn 1: Thu thập danh sách liên kết (link\_crawler.py)}
...
\subsubsection{Giai đoạn 2: Thu thập chi tiết từng xe (detail\_scraper.py)}
...
\subsubsection{Giai đoạn 3: Upload lên Google Cloud Storage (gcs\_uploader.py)}
...
\subsection{Điều phối tự động bằng Temporal}
...
\section{Xử lý và chuyển đổi dữ liệu (dbt Pipeline)}
...
\subsection{Nạp dữ liệu vào tầng Bronze (load\_bronze)}
...
\subsection{Chuyển đổi dữ liệu bằng dbt (Medallion Architecture)}

Sau khi dữ liệu đã nằm trong tầng bronze, \textbf{dbt} (data build tool) tự động chuyển đổi qua ba tầng:

\subsubsection{Staging (Silver Staging)}
...
\subsubsection{Silver (Mô hình chiều -- Dimensional Model)}

Tầng Silver tổ chức dữ liệu theo mô hình 3NF (Third Normal Form) gồm các bảng fact và dimension:
...
\subsubsection{Gold (Bảng phục vụ ứng dụng)}
...
\subsection{Refresh materialized view và tính toán ML}
...
\begin{itemize}
    \item \texttt{refresh\_matviews}: Làm mới các materialized view phục vụ hệ thống đề xuất (\texttt{mv\_popular\_vehicles}, \texttt{mv\_rating}).
...
\subsection{Tổng kết luồng xử lý dữ liệu}
...
\hline
6 & Silver: mô hình chiều 3NF & dbt (SQL table) \\
\hline
