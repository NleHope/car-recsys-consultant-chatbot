...
Hệ thống sử dụng \textbf{PostgreSQL} làm cơ sở dữ liệu quan hệ để lưu trữ các dữ liệu có cấu trúc như thông tin xe, người bán, giao dịch và metadata liên quan. PostgreSQL đảm bảo tính toàn vẹn dữ liệu, hỗ trợ các ràng buộc quan hệ và khả năng truy vấn phức tạp. Bên cạnh đó, \textbf{Qdrant} được sử dụng như một vector database, phục vụ cho bài toán tìm kiếm và gợi ý dựa trên vector embedding, cho phép hệ thống thực hiện các phép so sánh tương đồng (similarity search) hiệu quả trên không gian nhiều chiều. Để tối ưu hiệu năng và giảm tải cho backend cũng như database, \textbf{Redis} được sử dụng làm hệ thống cache, lưu trữ các kết quả truy vấn thường xuyên hoặc dữ liệu tạm thời.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.8\linewidth]{diagram.png}
    \caption{Kiến trúc tổng thể hệ thống backend}
    \label{fig:system-architecture}
\end{figure}

Hệ thống được thiết kế theo mô hình \textbf{microservices}, trong đó mỗi thành phần đảm nhiệm một vai trò riêng biệt và có thể triển khai, mở rộng độc lập. Frontend Service chạy trên cổng \textbf{3000}, chịu trách nhiệm hiển thị giao diện và tương tác trực tiếp với người dùng. Backend API chạy trên cổng \textbf{8000}, cung cấp các endpoint để xử lý nghiệp vụ và điều phối dữ liệu giữa các dịch vụ. PostgreSQL database hoạt động trên cổng \textbf{5432}, Qdrant vector database trên cổng \textbf{6333}, và Redis cache trên cổng \textbf{6379}.

Toàn bộ các service được đóng gói và triển khai bằng \textbf{Docker}, giao tiếp với nhau thông qua \textbf{Docker bridge network}. Cách tiếp cận này giúp hệ thống dễ dàng triển khai trên nhiều môi trường khác nhau, đảm bảo tính nhất quán giữa môi trường phát triển và môi trường triển khai, đồng thời tạo tiền đề thuận lợi cho việc mở rộng hoặc tích hợp thêm các dịch vụ mới trong tương lai.
...
\textbf{Client Layer} bao gồm ứng dụng frontend được xây dựng bằng React, chạy trên cổng 3000. Lớp này chịu trách nhiệm hiển thị giao diện người dùng, thu thập dữ liệu đầu vào và gửi các yêu cầu đến backend thông qua các REST API. Client không xử lý logic nghiệp vụ phức tạp mà tập trung vào trải nghiệm người dùng và tương tác.

\textbf{Backend Layer} đóng vai trò trung tâm điều phối và xử lý nghiệp vụ của hệ thống. Backend được xây dựng trên nền tảng FastAPI, chạy trên cổng 8000, cung cấp các endpoint RESTful cho frontend. Bên trong backend, các chức năng được chia thành nhiều service độc lập nhằm tuân thủ nguyên tắc single responsibility:
\begin{itemize}
...
\textbf{Data Layer} chịu trách nhiệm lưu trữ và truy xuất dữ liệu, bao gồm nhiều thành phần chuyên biệt cho từng loại dữ liệu. PostgreSQL được sử dụng làm cơ sở dữ liệu quan hệ để lưu trữ dữ liệu có cấu trúc như thông tin xe, người dùng và người bán. Redis được sử dụng làm hệ thống cache, giúp lưu trữ dữ liệu tạm thời và kết quả truy vấn thường xuyên nhằm giảm độ trễ và tăng hiệu năng. Qdrant đóng vai trò là vector database, phục vụ cho các bài toán tìm kiếm và gợi ý dựa trên độ tương đồng của vector embedding.
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=1\linewidth]{database_schema_architecture.png}
...
\noindent \textbf{Silver Schema} do dbt tạo, tổ chức theo mô hình 3NF (Third Normal Form):
\begin{itemize}
...
\begin{itemize}
    \item \textbf{Bảng do dbt tạo (marts):} \texttt{vehicles} (denormalized, merge by VIN — bảng chính phục vụ frontend), \texttt{vehicle\_price\_history} (partitioned by month), \texttt{car\_models}, \texttt{sellers}, \texttt{reviews}, \texttt{vehicle\_features}, \texttt{vehicle\_images}. Kèm theo các materialized view: \texttt{mv\_popular\_vehicles} và \texttt{mv\_rating}.
...
\section{Hệ thống Authentication}
...
\section{Caching và Tối ưu hiệu năng}

Để đảm bảo hiệu năng và khả năng mở rộng, hệ thống áp dụng cơ chế caching nhiều tầng với \textbf{Redis} làm trung tâm, kết hợp tối ưu truy vấn ở tầng cơ sở dữ liệu.

\subsection{Caching bằng Redis}

Hệ thống sử dụng Redis với TTL mặc định là \textbf{1 giờ} (\texttt{CACHE\_TTL = 3600}), bao gồm các lớp cache sau:

\begin{itemize}
    \item \textit{API Response Cache}: Lưu trữ kết quả của các truy vấn tìm kiếm phổ biến — các request có cùng tập tham số lọc sẽ được phục vụ trực tiếp từ cache, giảm tải cho PostgreSQL và Qdrant.
    \item \textit{Recommendation Cache}: Lưu kết quả gợi ý xe cho từng người dùng. Cache được chủ động \textit{invalidate} khi hệ thống ghi nhận tương tác mới (click, save, contact, v.v.), đảm bảo kết quả gợi ý phản ánh hành vi mới nhất.
\end{itemize}

\subsection{Tối ưu truy vấn Database}
...
Đối với các truy vấn tổng hợp phức tạp, hệ thống sử dụng \textbf{materialized views} — \texttt{mv\_popular\_vehicles} (xe phổ biến theo lượt tương tác) và \texttt{mv\_rating} (điểm đánh giá trung bình) — được refresh tự động mỗi lần pipeline chạy. Cách tiếp cận này giúp giảm chi phí tính toán tại thời điểm truy vấn và đảm bảo hiệu năng ổn định cho các API thường xuyên được gọi.
...
\subsection{Tóm tắt luồng xử lý}
