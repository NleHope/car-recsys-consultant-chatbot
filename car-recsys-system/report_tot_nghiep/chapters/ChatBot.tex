...
\subsection{Phạm vi tư vấn của hệ thống}
...
\subsection{Hạn chế của mô hình RAG truyền thống (Naive RAG)}
...
\end{figure}
\FloatBarrier
\subsection{Tổng quan về Agentic RAG}
...
\subsection{Sơ đồ kiến trúc tổng thể của hệ thống Agentic RAG}
...
\subsection{Quy trình thu thập và xử lý dữ liệu (Data Ingestion)}
...
\subsection{Quy trình tương tác với người dùng và Luồng xử lý hội thoại}
...
\end{enumerate}
\section{Thiết kế luồng Agentic RAG bằng LangGraph}
...
\subsection{Khởi tạo tài nguyên và Chu trình thực thi (Execution Cycle)}
...
\subsection{Định nghĩa Trạng thái Hội thoại (AgenticState)}
...
\end{tabular}
\end{table}
\subsection{Cơ chế quản lý hồ sơ khách hàng và Thu thập thông tin (Slot-filling)}
...
\subsubsection{Cấu trúc của Hồ sơ khách hàng}
...
\subsubsection{Quá Trình Cập Nhật}
...
\subsubsection{Hổ trợ cơ chế kiểm soát điều kiện tối thiểu khi tư vấn gợi ý xe}
...
\subsection{Kiến trúc Sơ đồ Đồ thị LangGraph}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=1\linewidth]{images/GraphState.drawio.png} 
    \caption{Sơ đồ luồng xử lý và định tuyến rẽ nhánh trong mạng đồ thị LangGraph}
    \label{fig:langgraph-flow}
\end{figure}
\FloatBarrier

\subsubsection{Cấu trúc các Nút xử lý (Nodes)}
...
\subsubsection{Cơ chế Định tuyến rẽ nhánh (Routing Logic)}
...
\subsection{So sánh với thiết kế SELF-RAG / Adaptive-RAG}
...
\section{Đánh giá mô hình (Evaluation)}

\subsection{Mục tiêu và Phương pháp luận}
...
\subsection{Xây dựng Bộ dữ liệu kiểm thử (Test Dataset)}
...
\subsection{Hệ thống Tiêu chí Đánh giá GEval theo Luồng Nghiệp vụ}
...
\begin{figure}[!htbp]
    \centering
    % Nhớ thay tên file 'deepeval_chart.png' thành tên file ảnh thực tế của bạn
