...
\centering
\small
\caption{Bảy loại intent của chatbot và ví dụ minh họa}
\label{tab:chatbot-intents}
\begin{tabular}{|l|l|l|}
\hline
...
\begin{itemize}[noitemsep, topsep=2pt]
    ...
    \item \textbf{Persistence chỉ áp dụng cho người dùng đã đăng nhập:} Lịch sử hội thoại của người dùng đã đăng nhập được ghi vào \texttt{gold.chat\_sessions} và \texttt{gold.chat\_messages} (cho phép khôi phục phiên cũ), trong khi phiên của khách vãng lai (guest) chỉ tồn tại in-memory và mất khi backend restart.
...
\begin{itemize}
    \item \textbf{Persistent UserProfile:} Ghi \texttt{UserProfile} (hiện đang in-memory) vào database để hỗ trợ triển khai đa instance (multi-instance) và khôi phục ngữ cảnh hồ sơ người dùng, bổ sung cho phần lịch sử hội thoại đã được lưu.
